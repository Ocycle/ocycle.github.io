...
function buat_link_halaman($id){
    ...
    // http://localhost/Projek%nunu/projek%web/EnviShare/halaman.php/8/judul
    return url_dasar()."/halaman.php/$id/$judul";
}


//buat halaman 2
function bersihkan_name($name){
    $name_baru     = strtolower($name);
    $name_baru     = preg_replace("/[^a-zA-Z0-9\s]/","",$name_baru);
    $name_baru     = str_replace(" ","-",$name_baru);
    return $name_baru;
}
function set_isii($isii){
    $isii    = str_replace("../gambar/",url_dasar()."/gambar/",$isii);
    return $isii;
}
function ambil_nama($id_tulis){
    global $koneksi;
    $sql1   = "select * from tutors where id = '$id_tulis'";
    $q1     = mysqli_query($koneksi,$sql1);
    $r1     = mysqli_fetch_array($q1);
    $name   = $r1['nama'];
    return $name;
...
function maximum_kata($isi,$maximum){
 ...
 function bersihkan_nama($nama){
    $nama_baru     = strtolower($judul);
    $nama_baru     = preg_replace("/[^a-zA-Z0-9\s]/","",$nama_baru);
    $nama_baru     = str_replace(" ","-",$nama_baru);
    return $judul_baru;
}
function buat_link_nama($id){
    global $koneksi;
    $sql1    = "select * from tutors where id = '$id'";
    $q1     = mysqli_query($koneksi,$sql1);
    $r1     = mysqli_fetch_array($q1);
    $nama  = bersihkan_nama($r1['nama']);
    // http://localhost/Projek%nunu/projek%web/EnviShare/halaman.php/8/judul
    return url_dasar()."/campaign.php/$id/$nama";
}
function ngambil_isi($id_tulisan){
    global $koneksi;
    $sql1   = "select * from tutors where id = '$id_tulisan'";
    $q1     = mysqli_query($koneksi,$sql1);
    $r1     = mysqli_fetch_array($q1);
    $text   = strip_tags($r1['isi']);
    return $text;
...
function buat_link_tutors($id){
    ...
    return url_dasar()."/halaman2.php/$id/$nama";
}

//tutors berakhir disini
